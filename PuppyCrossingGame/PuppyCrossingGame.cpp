...
int main() {
	...
	Global::window = GetConsoleWindow();
	RECT rect;
	rect.top = 0, rect.left = 0;
	rect.right = 820, rect.bottom = 2000;
	MoveWindow(Global::window, 1230, 600, 300, 200, TRUE);
	Global::hdc = GetDC(Global::window);
	PlaySoundW(L"resources/sound/music.wav", NULL, SND_FILENAME | SND_ASYNC | SND_LOOP);
	...
	Global::default_render_state = RenderState(720, 1280);

	// Global init
	Global::drawer.set_render_state(Global::default_render_state);
	Global::window_should_close = false;
	initShape();
